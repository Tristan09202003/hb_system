...
<script>
    ...
    function addReading(bpm) {
        ...
        document.getElementById('logBody').innerHTML = readings.slice(0, 20).map(r =>
            `<tr><td>#${r.id}</td><td>${r.bpm}</td><td class="st-${r.cls}">${r.lbl}</td><td>${r.time}</td></tr>`
        ).join('');
    ...
    function fetchLatest() {
        fetch('/api/get_data.php')
            .then(r => r.json())
            .then(data => { if (data && data.bpm) addReading(data.bpm); })
            .catch(() => {});
    }
